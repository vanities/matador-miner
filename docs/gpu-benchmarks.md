...
**`value` = thousands of nonce/s per Vast `$/hr`** - the metric that matters when renting.
Only hosts with >=8 effective vCPU are used (fewer starve the GPU and tank the rate). Vast
`$/hr` are per-offer snapshots from June 2026 and float with the marketplace; all cards ran
at 90-100% GPU util with 0 rejected shares.

## All NVIDIA GPUs (June 2026, sorted by value)
