...
is_safe() {
  local bi mi H HD IBD ntp reason canon
  ...
  log "h=$H/$HD ibd=$IBD near_tip_peers=$ntp reason=${reason:-?} canonical=${canon:-n/a}"
  [ "$IBD" = "true" ] && return 1                                  # still syncing
  [ -n "$H" ] && [ "$H" -ge "$((HD-1))" ] 2>/dev/null || return 1  # caught up to own headers
  [ "$ntp" -ge "$MIN_PEERS" ] 2>/dev/null || return 1             # well-peered (propagation)
  [ "$reason" = "healthy" ] || return 1                            # not behind/paused/reorg
  [ -n "$canon" ] && { [ "$H" -ge "$((canon-2))" ] 2>/dev/null || return 1; }  # caught up to NETWORK
  return 0
}
